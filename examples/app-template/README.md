...
| `dist/<id>.app.js` | 빌드 결과 — 로컬 미리보기용 |
| `dist/<id>.aihubapp.json` | **제출용 파일 하나** — 메타데이터와 코드가 함께 들어 있습니다 |
...
마켓플레이스 → **앱 제출** → `dist/<id>.aihubapp.json` 선택 → 심사 요청

이 파일 하나에 메타데이터와 코드가 함께 들어 있어, 고르면 항목이 자동으로 채워집니다.
따로 적을 것이 없습니다.

관리자가 코드를 읽고 승인하면 **마켓플레이스에 등록**되어 다른 사람이 설치할 수 있게 됩니다.
