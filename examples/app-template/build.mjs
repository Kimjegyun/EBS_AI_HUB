...
async function run() {
  ...
  // 제출 화면에 그대로 옮겨 적을 값을 함께 남긴다.
  writeFileSync(
    join(here, 'dist', 'submit-info.json'),
    JSON.stringify({ ...manifest, size: Buffer.byteLength(code) }, null, 2),
  )

  console.log(`\n✓ ${outFile}`)
  console.log(`  ${(Buffer.byteLength(code) / 1024).toFixed(1)} KB`)
  console.log('\n다음: 허브 → 마켓플레이스 → «로컬 미리보기» 로 확인한 뒤 «앱 제출»')
}
