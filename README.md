...
| | 내장 앱 | 원격 앱 |
|---|---|---|
| 방법 | 저장소에 파일 추가 + 레지스트리 한 줄 → PR | 관리자 화면에서 **번들 업로드** |
| 허브 재빌드 | 필요 | **불필요** |
| 배포 범위 | 이 저장소를 쓰는 모두 | 그 서버를 쓰는 조직 |

널리 쓰일 앱은 PR로 등록해 모두가 받아 쓰고, 조직 전용 앱이나 빠른 실험은
**번들을 올리는 것만으로 바로 얹습니다.**

만든 앱은 **파일 하나로 내보내 다른 회사 허브에 그대로 붙일 수 있습니다.**
받는 쪽은 `가져오기` 한 번이면 되고, 메타데이터를 입력하거나 빌드할 필요가 없습니다.
각자 만든 앱이 조직 경계를 넘어 쌓이게 하려는 장치입니다. 만드는 방법은 [doc/REMOTE_APPS.md](doc/REMOTE_APPS.md),
최소 예제는 [examples/remote-apps/hello-world.app.js](examples/remote-apps/hello-world.app.js) 를 보세요.
...
- [doc/REMOTE_APPS.md](doc/REMOTE_APPS.md) — **원격 앱 만들기 (재빌드 없이 앱 추가)**
