...
export function tencentAuthHeaders(apiKey: string, protocol?: 'completions' | 'responses' | 'messages') {
  const token = normalizeTencentApiKey(apiKey)
  const problem = describeTencentKeyProblem(token)
...
export function tencentKeyShape(raw: string): string {
  const key = normalizeTencentApiKey(raw)
  if (!key) return '비어 있음'
  return `길이 ${key.length}, 접두사 ${key.slice(0, Math.min(6, key.length))}…`
}

export function describeTencentHeaderError(err: unknown): string | null {
  const message = err instanceof Error ? err.message : String(err)
  if (/ByteString|greater than 255/i.test(message)) {
    return '키에 한글이 섞여 있어 인증 헤더를 만들 수 없습니다. Token 값만 다시 붙여넣고 저장하세요.'
  }
  return null
}
