...
create type public.ai_hub_session_result as (
  user_id uuid,
  project_id uuid,
  email text,
  display_name text,
...
create type public.ai_hub_member_result as (
  user_id uuid,
  project_id uuid,
  email text,
  display_name text,
...
returns setof public.ai_hub_session_result
language sql
stable
security definer
...
begin
  ...
  return query
  select * from public.get_ai_hub_session();
end
$$;

create or replace function public.list_ai_hub_members()
returns setof public.ai_hub_member_result
language sql
stable
security definer
set search_path = public, core, ai_hub
